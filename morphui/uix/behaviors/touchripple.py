...
class MorphRippleBaseBehavior(EventDispatcher):
    ...
    ripple_color: List[float] = ColorProperty([0.75, 0.75, 0.75, 0.5])
    ...
    def on_touch_down(self, touch: MotionEvent) -> bool:
        """Handle touch down events to initiate the ripple effect.

        This method is called when a touch event begins on the widget. If
        the touch event occurs within the widget's bounds and the ripple
        effect is enabled, the ripple effect will be shown.

        Parameters
        ----------
        touch : MotionEvent
            The touch event that triggered the ripple effect.
        ...
        Notes
        -----
        This implementation is based on Kivy ripple behavior's, see
        :meth:`~kivy.uix.behaviors.TouchRippleButtonBehavior.on_touch_down`
        """
        ...
        if self.ripple_enabled:
            self.show_ripple_effect(touch)
        self.dispatch('on_press')
        return True

    def on_touch_move(self, touch: MotionEvent) -> bool:
        """Handle touch move events to update the ripple effect position.
        
        This method is called when a touch event moves on the widget. If
        the ripple effect is in progress and the touch moves outside the
        widget's bounds, the ripple animation will be finished.
        ...
        -----
        This implementation is based on Kivy ripple behavior's, see
        :meth:`~kivy.uix.behaviors.TouchRippleButtonBehavior.on_touch_move`
        """
        if all((
                not self.collide_point(touch.x, touch.y),
                self._ripple_in_progress,
                not self._ripple_is_finishing,)):
    ...
    def on_touch_up(self, touch: MotionEvent) -> bool | None:
        """Handle touch up events to finalize the ripple effect.

        This method is called when a touch event ends on the widget. If
        the touch event was grabbed by the widget, the ripple effect will
        be finalized and the release action will be triggered.
        ...
        -----
        This implementation is based on Kivy ripple behavior's, see
        :meth:`~kivy.uix.behaviors.TouchRippleButtonBehavior.on_touch_up`
        """
        if touch.grab_current is not self:
            return super().on_touch_up(touch)
        
        assert self in touch.ud, (
            'Widget must be in touch.ud if it has grabbed the touch')
        ...
        release_delay = self.ripple_duration_out if self.ripple_enabled else -1
        Clock.schedule_once(
    ...
    def ripple_canvas_instructions(self) -> None:
        """Define the canvas instructions for rendering the ripple 
        effect.

        This method should be implemented by subclasses to specify how
        the ripple effect is drawn on the canvas. It should create the
        necessary instructions for rendering the ripple effect.
        """
        pass
...
class MorphCircularRippleBehavior(MorphRippleBaseBehavior):
    """Circular ripple effect mixin for Kivy widgets.

    Extends MorphRippleBaseBehavior to provide a material design-style
    circular ripple animation that expands from the touch point. The
    ...
    Examples
    --------
    Create a button with circular ripple effect:

    ```python
    class RippleButton(MorphCircularRippleBehavior, Label):
        pass
    ...
    def ripple_canvas_instructions(self) -> None:
        """Define the canvas instructions for rendering the circular
        ripple effect.

        This method creates the necessary canvas instructions to draw
        a circular ripple effect that expands from the touch point.

        Notes
        -----
        - The ripple is drawn using an Ellipse instruction, with its
          size determined by the current ripple radius.
